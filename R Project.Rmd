...
# for shiny app
listings.df %>%
  select(c(1, 7, 14, 19, 21:27, 29, 36, 39:45, 48:49)) %>%
  mutate(across(c(beds, bedrooms, bathrooms, accommodates, city, state, 
                  price, host_total_listings_count), as.factor)) %>%
  write.fst('./listings.fst')

reviews.df %>%
  mutate(across(c(id, price, accommodates, bathrooms, bedrooms, beds, 
                  month, year, quarter), as.factor)) %>%
  write.fst('./reviews.fst')
```

```{r}
# for shiny app
write_csv(listings.df, './listings_full.csv')
write_csv(reviews.df, './reviews_full.csv')
```
...
amenities.df %>%
  ...
  ggsave('Offered Amenities - Barplot.png', width = 10, height = 6)
```
...
listings.df %>%
  ...
  labs(title = 'Price Distribution by Property', 
       x = 'Price (USD)', 
       y = 'Density')
ggsave('Property Distribution - Density.png', width = 10, height = 6)
```
...
listings.df %>%
  ...
  labs(title = 'Competition among AirBNB Hosts',
       x = 'Year',
       y = 'New Hosts')
  ggsave('Competition among hosts year - Barplot.png', width = 10, height = 6)
...
listings.df %>%
  na.omit(host_since) %>%
  mutate(host_since = as.character(month(host_since))) %>%
  ggplot(aes(reorder(host_since, as.integer(host_since)),
             number_of_reviews)) + 
  geom_violin(aes(fill = airbnb_red), show.legend = FALSE) +
  # geom_smooth() + 
  ...
  labs(title = 'Competition among AirBNB Hosts',
       x = 'Month',
       y = 'New Hosts')
  ggsave('Competition among hosts month - violin.png', width = 10, height = 6)
```
...
reviews.df %>%
  ...
  labs(title = 'Accommodation Requests',
       x = 'Number of Guests',
       y = 'Count')
  ggsave('Accommodation Requests - Barplot.png', width = 10, height = 6)
```
