...
# for shiny app
listings.df %>%
  select(c(1, 7, 14, 19:27, 29, 36, 39:45, 48:49)) %>%
  # mutate(across(c(city, state), as.factor)) %>%
  write.fst('./listings.fst')

reviews.df %>%
  mutate(across(c(id, quarter, year, accommodates), as.factor), 
         day = day(date)) %>%
...
read.fst('./listings.fst') %>%
  ...
  ggplot(aes(price, host_total_listings_count)) +
...
listings.df %>%
  ...
  ggsave('./visualizations/Reviews by City - Barplot.png', width = 10, height = 6)
```
...
listings.df %>%
  ...
  labs(title = 'Competition among AirBNB Hosts',
       x = 'Month',
       y = 'New Hosts')
  ggsave('./visualizations/Competition among hosts month - violin.png', width = 10, height = 6)
```
...
reviews.df %>%
  ...
  labs(title = 'Seasonal Nightly Costs',
       x = 'Month',
       y = 'Count')
  ggsave('./visualizations/Seasonal Cost - Scatterplot.png', width = 10, height = 6)
```
